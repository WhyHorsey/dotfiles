...
ver() {
	...
	"redhat")
		...
		;;
	"suse")
		bat /etc/SuSE-release
		;;
	"debian")
		lsb_release -a
		;;
	"gentoo")
		bat /etc/gentoo-release
		;;
	"arch")
		bat /etc/os-release
		;;
	"slackware")
		bat /etc/slackware-version
		;;
	*)
		if [ -s /etc/issue ]; then
			bat /etc/issue
...
alias updateall='sudo dnf upgrade --refresh && brew update && brew upgrade && flatpak update'
alias man='batman'
alias cleanall='sudo dnf clean all && sudo dnf autoremove && brew cleanup && brew services cleanup && brew autoremove && flatpak uninstall --unused && sudo journalctl --vacuum-time=14d'
alias please='sudo !!'
alias le='eza --all --long --icons'
alias lek='eza --oneline --reverse --sort=size --all --long --icons'
alias ltg='eza --long --tree --level=3 --all --git --git-repos --icons'
alias lt='eza --long --tree --level=3 --all --git-ignore --git --git-repos --icons'
